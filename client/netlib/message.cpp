...
bool DoMsgSend_negtiate(int type,int subtype,char *pdata,int datalen)
{
	MSGHEAD head;
	...
	if (g_cfg.sendcmd(&head,pdata,(SOCKADDR *)&dst_addr) == false)
	{
		return false;
	}
...
bool DoMsgSend(int type,int subtype,char *pdata,int datalen)
{
	MSGHEAD head;
	SOCKADDR_IN dst_addr;
	memset(&head,0,sizeof(head));
