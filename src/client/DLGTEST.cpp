...
void CDLGTEST::OnBnClickedButton1()
{
	...
	char *pdata = new char[sizeof(ANSWER)*count];
	memcpy(pdata,(char *)panswer,sizeof(ANSWER)*count);
	if (DoMsgSend(MSG_MANAGE,MSG_TEST_COMMIT,pdata,sizeof(ANSWER)*count) == true)
		rtnflg = true;
	else
		rtnflg = false;
	memset(&slog,0,sizeof(SLOG));
	strcpy(slog.username,(char *)(LPCTSTR)g_name);
	slog.msgtype = MSG_TEST_COMMIT;
	time = CTime::GetCurrentTime();
	strtime.Format("%d-%d-%d %d:%d",time.GetYear(),time.GetMonth(),time.GetDay(),time.GetHour(),time.GetMinute());
	strcpy(slog.time,(char *)(LPCTSTR)strtime);
	if (rtnflg == true)
		strcpy(slog.msg,"成功提交考试信息！");
	else
		strcpy(slog.msg,"提交考试信息失败！");
	//调用DoMsgSend()向服务器发送日志信息
	DoMsgSend(MSG_MANAGE, MSG_LOG, (char *)&slog, sizeof(SLOG));
